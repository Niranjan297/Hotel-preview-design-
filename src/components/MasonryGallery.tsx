...
export const MasonryGallery: React.FC<MasonryGalleryProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    grid.forEach((item, index) => {
      const element = document.querySelector(`[data-masonry-key="${item.id}"]`) as HTMLElement;
      if (!element) return;
    ...
    >
      {grid.map(item => (
        <div
          ...
          className={cn(
            'absolute overflow-hidden cursor-pointer rounded-2xl group',
            itemClassName
          )}
        ...
        >
          ...
          >
            {colorShiftOnHover && (
              <div className="color-overlay absolute inset-0 bg-gradient-to-tr from-brand-gold/40 to-brand-charcoal/40 opacity-0 pointer-events-none" />
            )}
          </div>
          {item.title && (
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-brand-charcoal/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <p className="text-brand-ivory text-xs font-serif italic tracking-wider">{item.title}</p>
            </div>
          )}
        </div>
      ))}
